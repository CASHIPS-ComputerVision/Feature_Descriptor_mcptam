...
{ //BundleAdjust(spAdjustSet, spFixedSet, spMapPoints, vOutliers, false);
...
//   std::map<int, std::string> mmBundleID_CamName;  ///< %Map FROM camera id TO camera name
  //cout<<"BundleAdjusterSingle::BundleAdjust"<<endl;
  mmPoint_BundleID.clear();
  mmBundleID_Point.clear();
  mmKF_BundleID.clear();
  mmBundleID_KF.clear();
  
  if((int)spMapPoints.size() < BundleAdjusterBase::snMinMapPoints)//10
  {
