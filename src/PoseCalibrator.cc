...
: SystemBase("PoseCalibrator",true, true)
{
 ...
 // GUI.ParseLine("Menu.AddMenuToggle Images \"Draw Masks\" DrawMasks Images");
  ...
  mNodeHandlePriv.param<string>("pattern_location", location_value,"");
  ofstream mylocation("location.txt");
  location_value+="/stand.jpg";
  mylocation<<location_value;
  mylocation.close();
  cout<<"--------------------------------------------------------------------------------------------------------------------"<<location_value<<endl;
...
std::string PoseCalibrator::Track()
{
  ...
  cout<<"bDrawKeyFrames = *gvsCurrentSubMenu == "<<bDrawKeyFrames <<endl;
  if(bGotNewFrame)
  {
    ...
    for(ImageBWMap::iterator it = mmFramesBW.begin(); it != mmFramesBW.end(); it++, nCamNum++)
    {
      ...
      // If this tracker has not yet found a checkerboard, and none of the trackers in this loop has tried to initialize yet,
    ...
    if(bNeedToDrop)  // Did any of the trackers want to add to map?
    {
      ...
      if(!mpMap->mbGood) // map has not been initialized yet with calibration pattern points
      {
        ...
        cout<<"any of the trackers want to add to map?---"<<firstCamName<<endl;
        // Call map maker's init from calib image
        bool bSuccess = mpMapMaker->InitFromCalibImage(*(pFirstTracker->mpCalibImage), mdSquareSize, firstCamName, pFirstTracker->mpCurrentMKF->mse3BaseFromWorld);
	...
	pFirstTracker->MarkKeyFrameAdded();
      ...
      {       cout<<"There is a good mapThere is a good mapThere is a good map"<<endl;
        ...
        else  // Otherwise we are too eager, don't add a new MKF yet 
        {
          ...
          // However, at the beginning of TrackerCalib's TrackFrame all measurements and backlinks are cleared, so we don't have to do anything here 
        
          // Still need to clear the need to drop flag otherwise TrackerCalib will think we didn't attend to its needs and
          // throw an assertion
...
std::string PoseCalibrator::Optimize()     
{
  ...
  if(bSuccess)
  {
    for(SE3Map::iterator it = mmFinalPoses.begin(); it != mmFinalPoses.end(); it++)
    {
      ...
      tf::Matrix3x3 rot(m3Rot(0,0),m3Rot(0,1),m3Rot(0,2),m3Rot(1,0),m3Rot(1,1),m3Rot(1,2),m3Rot(2,0),m3Rot(2,1),m3Rot(2,2));
      double r,p,y;
      rot.getRPY(r,p,y);
...
void PoseCalibrator::TransferKeyFrame(MultiKeyFrame* pMKF, TrackerCalib* pCT)
{
  ...
  pCT->mpCurrentMKF->mmpKeyFrames.erase(camName); // first remove our KF from list of keyframes so destructor of MKF won't try to delete KF
