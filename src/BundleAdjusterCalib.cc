...
int BundleAdjusterCalib::BundleAdjust(std::set<MultiKeyFrame*> spAdjustSet, std::set<MultiKeyFrame*> spFixedSet, std::set<MapPoint*> spMapPoints, std::vector<std::pair<KeyFrame*, MapPoint*> >& vOutliers, bool bRecent)
{
  cout<<"BundleAdjusterCalib::BundleAdjust"<<endl;
  if(!spFixedSet.empty()) // Shouldn't have any MKFs forced to be fixed when using BundleAdjusterCalib
  {
    ROS_FATAL("BundleAdjusterCalib: Got a non-empty fixed set, you should not call BundleAdjustRecent(...) as the whole map "
  ...
  {
    ROS_INFO_STREAM("BundleAdjusterCalib: Not enough good map points, got "<<spMapPoints.size()<<", need "<<BundleAdjusterBase::snMinMapPoints);
  ...
  for(SE3Map::iterator se3_it = ++mRelativePoses.begin(); se3_it != mRelativePoses.end(); ++se3_it)
  {
    ...
    ROS_DEBUG_STREAM("From ExtractRelativePoses, "<<camName<<std::endl<<se3Pose);
  ...
  for(std::set<MapPoint*>::iterator point_it = spMapPoints.begin(); point_it!=spMapPoints.end(); point_it++)
  {
    ...
    else
    {
      ...
      if(mmCamName_BundleID.count(point.mpPatchSourceKF->mCamName))  // this is a relative camera, camera chain is 2 poses long
  ...
  for(std::map<MultiKeyFrame*, int>::iterator mkf_it = mmBase_BundleID.begin(); mkf_it != mmBase_BundleID.end(); ++mkf_it)
  {
    ...
    for(KeyFramePtrMap::iterator kf_it = mkf.mmpKeyFrames.begin(); kf_it != mkf.mmpKeyFrames.end(); ++kf_it)
    {
      ...
      for(MeasPtrMap::iterator meas_it = kf.mmpMeasurements.begin(); meas_it != kf.mmpMeasurements.end(); meas_it++)
      {
        ...
        calibBundle.AddMeas(vCams, nPoint_BundleID, meas.v2RootPos, LevelScale(meas.nLevel) * LevelScale(meas.nLevel), camName);
      }
    }
  }
  ...
  if(nAccepted < 0)
  {
    ...
    ROS_ERROR("BundleAdjusterCalib: Cholesky failure in bundle adjust. The map is probably corrupt: Should ditch the map.");
    return -1;
  }
