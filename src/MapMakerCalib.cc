...
bool MapMakerCalib::InitFromCalibImage(CalibImageTaylor &calibImage, double dSquareSize, std::string cameraName, SE3<> &se3TrackerPose)
{
  ...
  for(int l=0; l < LEVELS; ++l)
  {
    ...
    for(unsigned i=0; i < calibImage.mvGridCorners.size(); ++i)
    {
      ...
      //pNewPoint->mv3WorldPos.slice<0,2>() = dSquareSize * CVD::vec(calibImage.mvGridCorners[i].mirGridPos_Feature);
      pNewPoint->mv3WorldPos.slice<0,2>() = dSquareSize * (calibImage.mvGridCorners[i].mirGridPos_Feature);
  ...
  */
  ...
  mBundleAdjuster.SetNotConverged();//mbBundleConverged_Recent = false; mbBundleConverged_Full = false;
  ...
  while(!mBundleAdjuster.ConvergedFull())//mbBundleConverged_Full
  {
...
bool MapMakerCalib::CalibOneStep()
{
  ...
  cout<<"CalibOneStep HandleOutliers start"<<endl;
  HandleOutliers(vOutliers);
  cout<<"CalibOneStep HandleOutliers over"<<endl;
  return nAccepted >= 0;
}
