...
void MapMakerServer::run()
{
  ...
  while(!shouldStop() && ros::ok())  // ShouldStop is a CVD::Thread func which return true if the thread is told to exit.
  {
	...
	cout<<"localBA HandleOutliers start"<<endl;
        HandleOutliers(vOutliers);
	cout<<"localBA HandleOutliers over"<<endl;
    ...
    if(mBundleAdjuster.ConvergedRecent() && !mBundleAdjuster.ConvergedFull() && IncomingQueueSize() == 0)
    {
      ...
      {
        mnNumConsecutiveFailedBA = 0;
	cout<<"GlobalBA HandleOutliers start"<<endl;
        HandleOutliers(vOutliers);
	cout<<"GlobalBA HandleOutliers over"<<endl;
        mNetworkManager.SendOutliers(vOutliers);
        PublishMapVisualization();
        
        if(mState == MM_INITIALIZING && mdMaxCov < MapMakerServerBase::sdInitCovThresh) 
        {
          ROS_INFO_STREAM("MM_INITIALIZING, Max cov "<<mdMaxCov<<" below threshold "<<MapMakerServerBase::sdInitCovThresh<<", sending MM_RUNNING to client");
