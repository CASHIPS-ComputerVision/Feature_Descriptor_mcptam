...
public:
  ...
  void GuessInitialPose(TooN::Vector<2> v2Center);//没有使用过
  ...
  std::pair< TooN::Matrix<>, TooN::Vector<> > BuildIntrinsicMatrixEntries(TooN::Vector<2> v2Center, int nDegree, int nImgNum, int nTotalNum);//没有使用过
...
protected:
  ...
  //add by zjd
   cv::Mat standimageDesc;
   std::vector<cv::KeyPoint> standkeyPoint;
